        ...
        if (context.Result != null)
        {
            if (context.Result is ObjectResult)
            {
                ...
                if (objectResult?.GetType().Name == "BadRequestObjectResult")
                {
                    ...
                    {
                        Code = objectResult.StatusCode, Data = new { }, Message = objectResult.Value
                    });
                }
                else if (objectResult?.Value?.GetType().Name == "ModelStateResult")
                {
                    ...
                    context.Result = new JsonResult(new
                    {
                        modelStateResult?.Code, Data = new { }, modelStateResult?.Message
                    });
                }
                else
                {
                    context.Result = new JsonResult(new { Code = 200, Data = objectResult?.Value });
                }
            }
            else if (context.Result is EmptyResult)
            {
                context.Result = new JsonResult(new { Code = 200, Data = new { } });
            }
            else if (context.Result is ModelStateResult)
            {
                ...
                context.Result = new JsonResult(new
                {
                    modelStateResult2?.Code, Data = new { }, modelStateResult2?.Message
                });
            }
        }
